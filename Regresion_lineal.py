...
class PolinomiosDeLagrange (polinomios):
    ...
    def __init__(self, serie_de_puntos):
        ...
        self.x_values = [x[0] for x in self.serie_de_puntos]
        self.y_values = [y[0] for y in self.serie_de_puntos]
    ...
    #             if xi == xj:
    #                 continue
    #             numerator = polinomios(grado=1, coef=[-xj, 1]) # x - xj
    #             denominator = xi - xj
    #             monomial = numerator // denominator
    #             wi = monomial.__mul__(wi)
    #         prod = wi.__mul__(yi)
    #         lagrange_poly = lagrange_poly.__add__(prod)
        
    #     return lagrange_poly.coefs
    def get_lagrange(self):
        lagrange = polinomios(0,[0])
        for j, yj in enumerate(self.y_values):
            wi = 1
            for i, xi in enumerate(self.x_values):
                
                if j == i:
                    pass
                
                poli = polinomios(1, [-xi, 1])
                div = self.x_values[j]-xi
                monomio  = poli.__floordiv__(div if div!=0 else 1)
                wi = monomio.__mul__(wi)
            prod = wi.__mul__(yj)
            lagrange = lagrange.__add__(prod)
    ...
    def px(self):
        ...
        for i in self.serie_de_puntos:
            ...
            suma.append(productito)
        
        suma_final = suma[0]
        
        for i in suma:
            suma_final.__add__(i)
...
a=lagrangiano.get_lagrange()
a.get_expression()
a.poly_plt(-10, 10)

#%% Sub clase Regresion Lineal
class linreg (polinomios):
    ...
    def __init__(self, datos):
        ...
        self.x_mean = sum(self.datos[0])//len(self.datos)
        self.y_mean = sum(self.datos[1])//len(self.datos)
    ...
    def beta_MCO(self):
        resultado = 0
        for d in self.datos:
            producto = ((d[0]-self.x_mean)*((d[1])-self.y_mean))//((d[0]-self.x_mean)**2)
            resultado += producto
            return resultado
    
    def __str__(self):
        expresion = ["Y=", str(round(self.alpha)), "+", str(round(self.beta_MCO)), "X"]
